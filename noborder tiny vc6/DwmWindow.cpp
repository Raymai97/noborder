...
DwmWindow::DwmWindow()
{
	...
	this->hWnd = CreateWindowEx(
		...
		hInst,
		lpParam);
	if (this->hWnd == NULL)	{ MSGERR("FATAL: DwmWindow InitHwnd failed!"); PostQuitMessage(1); }
}
...
void DwmWindow::Start(Target *target, bool topMost)
{
	...
	this->ing = true;
	DWORD tid;
	CreateThread(NULL, 0, DwmWindow::CheckTargetProc, this, 0, &tid);
}
...
void DwmWindow::UpdateThumb()
{
	...
	props.fVisible = true;
	props.rcSource = psSrc.ToRECT();
	props.rcDestination = psDest.ToRECT();
