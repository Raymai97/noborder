...
void LoadConfig()
{
	...
	{
		DWORD byteCount;
		ReadFile(hFile, &excludeTaskbar, 1, &byteCount, nullptr);
		ReadFile(hFile, &alwaysOnTopMode, 1, &byteCount, nullptr);
		ReadFile(hFile, &useDWM, 1, &byteCount, nullptr);
		CloseHandle(hFile);
	}
}
...
void SaveConfig()
{
	...
	{
		DWORD byteCount;
		WriteFile(hFile, &excludeTaskbar, 1, &byteCount, nullptr);
		WriteFile(hFile, &alwaysOnTopMode, 1, &byteCount, nullptr);
		WriteFile(hFile, &useDWM, 1, &byteCount, nullptr);
		CloseHandle(hFile);
	}
}
